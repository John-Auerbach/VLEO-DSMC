...
# ============================================================================
# IONS (optional) - International Reference Ionosphere via the iri2016 package
# ============================================================================
n_Op = n_O2p = n_NOp = n_Np = 0.0
ne_val = Ti_val = Te_val = 0.0
if include_ions:
    # iri2016 builds a Fortran library on first use and needs CMake on PATH.
    ...
    iri_file = os.path.join(data_dir, 'iri.dat')
    if not os.path.exists(iri_file):
        ...
        try:
            ...
            print("ERROR: --ions requested but the 'iri2016' package is not installed.")
            print("       Install it in the active environment:")
            print("           python -m pip install iri2016 cmake")
            sys.exit(1)

        iono = iri2016.IRI(UTC, [70, 300, 1], LAT, LON)
...
out_path = os.path.join(data_dir, 'atm.sparta')
